...
94 files across 21 phases (Phase 15 added none — POS reuses `orders`' and
...
| 24    | `20260830160000_create_audit_permissions.sql`               | `audit.view` — the first new permission since Phase 20 (ADR-028)                    |
| 24    | `20260830160100_create_audit_logs.sql`                      | `audit_logs`, the redaction policy, and the only thing that writes it               |
| 24    | `20260830160200_create_audit_triggers.sql`                  | Fifteen triggers covering master §17's nine sensitive actions                       |
| 24    | `20260830160300_create_platform_diagnostics.sql`            | `platform_diagnostics()` for the Super Admin. **No table**                          |
| 25    | `20260831120000_create_rate_limits.sql`                     | `rate_limit_counters` + `consume_rate_limit()`; state a serverless deploy can share |
| 25    | `20260831120100_allow_product_detach_from_closed_order.sql` | KL-2308: a sold product can be deleted again. **No table**                          |
...
| 03    | `roles`, `permissions`, `role_permissions`                | select, `using (true)`                                                                                                                                | Read-only capability catalogue — no tenant data, so a blanket read is not an isolation hole. One of only two _exceptions_ to `using (true)` being forbidden (see below)                                                                                                                                                                                            |
...
| 22    | `subscription_events`                                     | select member or platform admin, **and nothing else**                                                                                                 | One of two tables with no write policy for anybody, platform admin included: five SECURITY DEFINER triggers are its only writers (ADR-026 decision 4)                                                                                                                                                                                                              |
| 24    | `audit_logs`                                              | select `audit.view` **or** platform admin, **and nothing else**                                                                                       | The other one. Fifteen SECURITY DEFINER triggers are its only writers, and the absence of an INSERT policy is exactly what makes a row here worth something — one somebody can write is one somebody can fabricate (ADR-028 decision 1)                                                                                                                            |
| 25    | `rate_limit_counters`                                     | **no policies at all** — not even select                                                                                                              | The third, and the strictest of the three: even reading is refused, because a readable counter answers "how many attempts does this address have left?" and a writable one locks somebody else out. Only `consume_rate_limit()` touches it. The one table here with no `tenant_id`, because it governs the surface before any tenant is known (ADR-029 decision 3) |
...
Phase 24 takes the same pattern to its widest: `audit_row_change()` fires from
**fifteen** triggers on eleven different tables and writes into `audit_logs`,
which likewise has no write policy for anybody. One generic writer rather than
fifteen copies, because the semantic action name arrives as `TG_ARGV[0]` — so
auditing a new action is a three-line trigger, not another function. It is also
the only trigger in the schema that reads the HTTP request: `audit_client_ip()`
and `audit_request_header()` pull the visitor's address, user agent and request
id out of PostgREST's `request.headers` GUC, which
`createSupabaseServerClient()` fills by forwarding them as
`x-clovercode-*` headers. Both helpers return `NULL` rather than raising when
there is no request behind the statement, so the worst case of that path is an
audit row without an IP — never a business write that fails
...
`has_billing_credentials`, `clear_billing_credentials`) are the _only_ code
